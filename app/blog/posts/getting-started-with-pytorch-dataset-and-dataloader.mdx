...
The best source for PyTorch tutorials is actually the PyTorch website itself. In fact, most of the tutorial I am going to give below is inspired by the quick start tutorial on the PyTorch website: [PyTorch Quick Start Tutorial](https://pytorch.org/tutorials/beginner/basics/quickstart_tutorial.html). That said, I hope the comments I am going to make and the explanations I am going to provide will help you understand the fundamentals of Torch deeply and will enable you to appreciate the framework the same way I do.
...
1. An `__init__` function to load your data or point to its location.
2. A `__len__` function to return the dataset size.
3. A `__getitem__` function to fetch one row from your dataset.
